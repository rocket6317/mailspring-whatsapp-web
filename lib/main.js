...
function configureMenuBarClick() {
...
function onMailboxClick(event) {
  if (!whatsappVisible || !(event.target instanceof Element)) return;
  if (event.target.closest('.item-action-button, .item-input')) return;
  if (event.target.closest('.account-sidebar-sections .item')) setWhatsAppVisible(false);
}
...
class MailSyncButton extends React.Component {
  render() {
    const title = 'Sync mail now';

    return React.createElement(
      'button',
      {
        className: 'btn btn-toolbar whatsapp-sync-button',
        title,
        'aria-label': title,
        onClick: syncMailNow,
      },
      React.createElement('span', { className: 'whatsapp-sync-icon', 'aria-hidden': 'true' })
    );
  }
}

MailSyncButton.displayName = 'MailSyncButton';

function activate() {
  configureWhatsAppPermissions();
  configureMenuBarClick();
  document.addEventListener('click', onMailboxClick, true);
  ...
  ComponentRegistry.register(MailSyncButton, {
    location: WorkspaceStore.Location.RootSidebar.Toolbar,
  });
}
...
function deactivate() {
  ...
  document.removeEventListener('click', onMailboxClick, true);
  ...
  ComponentRegistry.unregister(MailSyncButton);
}
